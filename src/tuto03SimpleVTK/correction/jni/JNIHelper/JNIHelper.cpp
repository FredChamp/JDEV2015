...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "JNIHelper", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "JNIHelper", __VA_ARGS__))
...
int JNIHelper::callIntMethod(const std::string& method)
{
    if( m_activity == NULL )
    {
        LOGE( "JNIHelper has not been initialized. Call init() to initialize the helper" );
        return 0;
    }
